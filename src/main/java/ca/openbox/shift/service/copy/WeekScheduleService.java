...
public class WeekScheduleService {
    ...
    public PresetResultDTO copyWeekSchedule(PresetRequestDTO presetRequestDTO) {
        //1. Validate Request
        //1.1 TODO：Validate srcWeekStart,tgtWeekStart, groupName, mode
        if (presetRequestDTO.getTgtWeekStart().isBefore(presetRequestDTO.getSrcWeekStart())) {
                throw new InvalidScheduleRangeException(
                        "Target week cannot be earlier than the source week"
                );
        }
        //2.Load Source Week Shifts
        //2.1 Global time zone handling
        ...
        //7. insert shifts
        for(ShiftArrangementDO generatedShiftDO : generatedShiftDOs) {
            shiftArrangementRepository.save(generatedShiftDO);
        }
        // 8. Build result
